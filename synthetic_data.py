...
# ─────────────────────────────────────────────────────────────────────────────
# Tidally LOCKED synthetic planet
# ─────────────────────────────────────────────────────────────────────────────

LOCKED_PARAMS = {
    "name":            "SyntheticA (Locked Demo)",
    "a_AU":            0.04,           # close-in — definitely locked
    "M_star_Msun":     0.5,
    "R_planet_Rearth": 1.2,
    "P_orb_days":      8.0,
    "P_rot_days":      8.0,            # = P_orb  → TIDALLY LOCKED
    "eccentricity":    0.00,
    "system_age_gyr":  8.0,            # old system -> deeply locked
    # Atmospheric shifts: symmetric, small (co-rotating atmosphere)
    # Morning limb has the same velocity as the orbital motion projected on LOS
    # Evening limb identically so -> Dv_atm ~ 0
    "v_ingress_km_s":  +0.10,          # tiny residual (GCM drag, model imperfection)
    "v_egress_km_s":   -0.10,          # symmetric -> net Dv ~ 0
    "v_err_km_s":       0.15,          # same instrument noise as unlocked demo
    "transit_depth":    0.0025,
    "transit_dur_hrs":  2.5,           # shorter transit (closer-in orbit)
    # RM effect geometry: star rotates slowly (old K dwarf), lambda = 0 (aligned)
    "v_sin_i_star_kms": 2.0,           # slow stellar rotation
    "lambda_deg":       0.0,           # perfectly aligned prograde orbit
}


class SyntheticLockedDataset:
    """
    Synthetic observables for a TIDALLY LOCKED planet.

    Physical scenario
    -----------------
    - Planet at 0.04 AU around a 0.5 M_sun K dwarf, P_orb = P_rot = 8 days
    - System age 8 Gyr >> tau_sync (deeply locked)
    - Atmosphere co-rotates with the planet: no net equatorial super-rotation
    - Ingress and egress Doppler shifts are SYMMETRIC: |Dv_ingress| ~ |Dv_egress|
      -> net Dv_atm ~ 0, unlike the unlocked case
    - Rossiter-McLaughlin anomaly: symmetric (lambda = 0, prograde orbit),
      blueshift during ingress, redshift during egress (blocking approaching
      then receding stellar disk)

    Contrasting with SyntheticTidalDataset (unlocked):
    - Unlocked: Dv_ingress = +1.8 km/s (redshift), Dv_egress = -2.2 km/s (blueshift)
      -> net Dv ~ 4 km/s (strong super-rotating jet signature)
    - Locked:   Dv_ingress = +0.1 km/s, Dv_egress = -0.1 km/s
      -> net Dv ~ 0 (co-rotating atmosphere, no jet signature)

    This contrast is the CORE observational diagnostic of the project.
    """

    def __init__(self, params=None, seed=99):
        self.p   = params or LOCKED_PARAMS.copy()
        self.rng = np.random.default_rng(seed)

    # ------------------------------------------------------------------
    def generate_rv_timeseries(self, n_points=80):
        """
        Synthetic RV time-series for the locked planet.

        The RM anomaly IS present (from stellar rotation), but:
        - It is symmetric (lambda = 0 -> prograde aligned orbit)
        - The atmospheric wind shift is nearly zero (Dv_atm ~ 0)
        - Ingress: planet crosses approaching side of star -> observed blueshift
        - Egress:  planet crosses receding side of star  -> observed redshift
        NOTE: the SIGN convention here is opposite to the unlocked case because
        the RM effect and the atmospheric wind signal have OPPOSITE signs for
        a locked vs unlocked planet when the orbit is prograde.
        """
        dur   = self.p["transit_dur_hrs"]
        t_hrs = np.linspace(-dur, dur, n_points)

        t_in_start = -dur / 2.0
        t_in_end   = t_in_start + dur * 0.15
        t_eg_start = dur / 2.0 - dur * 0.15
        t_eg_end   = dur / 2.0

        v_si  = self.p["v_sin_i_star_kms"]
        rv    = np.zeros(n_points)

        # RM anomaly from stellar rotation (symmetric for lambda=0)
        for i, t in enumerate(t_hrs):
            if t_in_start <= t <= t_in_end:
                frac   = (t - t_in_start) / (t_in_end - t_in_start)
                rv[i]  = -v_si * 0.4 * frac       # blueshift (approaching limb)
            elif t_in_end < t < t_eg_start:
                frac   = (t - t_in_end) / (t_eg_start - t_in_end)
                rv[i]  = -v_si * 0.4 + v_si * 0.8 * frac   # crosses zero
            elif t_eg_start <= t <= t_eg_end:
                frac   = (t - t_eg_start) / (t_eg_end - t_eg_start)
                rv[i]  = v_si * 0.4 * (1 - frac)  # redshift (receding limb)

        # Add small SYMMETRIC atmospheric shift (co-rotating -> nearly zero)
        for i, t in enumerate(t_hrs):
            if t_in_start <= t <= t_in_end:
                rv[i] += self.p["v_ingress_km_s"] * 0.4   # +0.04 km/s
            elif t_eg_start <= t <= t_eg_end:
                rv[i] += self.p["v_egress_km_s"] * 0.4    # -0.04 km/s

        rv_true = rv.copy()
        base_noise = self.p["v_err_km_s"]
        rv_err  = base_noise * (1.0 + 0.3 * self.rng.uniform(-1, 1, n_points))
        rv_obs  = rv + self.rng.normal(0, rv_err)
    ...
    def generate_transmission_spectra(self, n_pixels=500, line_center_nm=589.0):
        """
        Ingress vs. egress transmission spectra for the LOCKED planet.

        Key difference from unlocked: ingress and egress lines are at nearly
        the SAME wavelength — the atmosphere co-rotates with the planet,
        so there is no asymmetric Doppler shift.
        Any tiny shift (< 0.01 nm) is due to the planet's orbital motion
        projected onto the line of sight (which affects both limbs equally
        in a tidally locked configuration).
        """
        v_in = self.p["v_ingress_km_s"]   # +0.10 km/s (tiny)
        v_eg = self.p["v_egress_km_s"]    # -0.10 km/s (symmetric)

        dl_in = line_center_nm * v_in / C_KMS
        dl_eg = line_center_nm * v_eg / C_KMS
        ...
        delta_v = v_in - v_eg   # ~0.2 km/s (nearly zero)
    ...
    def print_summary(self):
        p = self.p
        print("\n" + "="*60)
        print("  SYNTHETIC DATASET -- Tidally-LOCKED Planet")
        print("="*60)
        print(f"  System:             {p['name']}")
        print(f"  Orbital semi-major: {p['a_AU']} AU")
        print(f"  Orbital period:     {p['P_orb_days']} days")
        print(f"  Rotation period:    {p['P_rot_days']} days  (= P_orb: LOCKED)")
        print(f"  System age:         {p['system_age_gyr']} Gyr  (deeply locked)")
        print(f"\n  Atmospheric Wind Signature (SYMMETRIC):")
        print(f"    Ingress (morning limb):  v = {p['v_ingress_km_s']:+.2f} km/s")
        print(f"    Egress  (evening limb):  v = {p['v_egress_km_s']:+.2f} km/s")
        dv = p['v_ingress_km_s'] - p['v_egress_km_s']
        print(f"    Delta_v:                 {dv:+.2f} km/s  -> Co-rotating atmosphere, NO jet")
        print(f"  RM effect:  v sin i = {p['v_sin_i_star_kms']} km/s, lambda = {p['lambda_deg']} deg")
        print("="*60)


# ─────────────────────────────────────────────────────────────────────────────
def compare_locked_unlocked(seed_locked=99, seed_unlocked=42):
    """
    Generate both locked and unlocked synthetic datasets for direct comparison.

    Returns
    -------
    dict with keys 'locked' and 'unlocked', each containing:
        rv_data, spec_data, params
    """
    locked_ds   = SyntheticLockedDataset(seed=seed_locked)
    unlocked_ds = SyntheticTidalDataset(seed=seed_unlocked)

    return {
        "locked": {
            "rv_data":   locked_ds.generate_rv_timeseries(),
            "spec_data": locked_ds.generate_transmission_spectra(),
            "params":    locked_ds.p,
        },
        "unlocked": {
            "rv_data":   unlocked_ds.generate_rv_timeseries(),
            "spec_data": unlocked_ds.generate_transmission_spectra(),
            "params":    unlocked_ds.p,
        },
    }
